...
export default function SolverView({
  ...
  function addTableauToPDF(
    ...
    doc.rect(14, y, tableW, rowH, "S");
    for (let c = 1; c < numCols; c++) {
      doc.line(14 + c * colW, y, 14 + c * colW, y + rowH);
    }
    doc.setTextColor(71, 85, 105);
    for (let c = 0; c < numCols; c++) {
      const x = 14 + c * colW;
      doc.text(colNames[c], x + colW / 2, y + rowH / 2 + fontSize * 0.18, {
        align: "center",
      });
    }
    ...
    for (let r = 0; r < tableau.matrix.length; r++) {
      ...
      const textY = y + rowH / 2 + fontSize * 0.18;

      // Basis cell
      doc.setFont("helvetica", "bold");
      doc.setTextColor(71, 85, 105);
      doc.text(basisNames[r] ?? "", 14 + colW / 2, textY, { align: "center" });
      ...
      for (let c = 0; c < tableau.matrix[r].length; c++) {
        ...
        const x = 14 + (c + 1) * colW;
        doc.text(val, x + colW / 2, textY, { align: "center" });
      }
